#!/usr/bin/env python3
"""
Deploy lora-v1: distill data → upload dataset → push kernel → poll status.

Usage:
    python run.py                          # default: Tier AB, full pipeline
    python run.py --tier ABC               # include Gemini reasoning
    python run.py --tier ABC --max-c 100   # limit Gemini to 100 samples
...
KERNEL_ID = "lasseruttert/nemotron-lora-v1"
...
def main() -> None:
    ...
    parser.add_argument("--tier", default="AB", help="Data tiers: A, B, C, or combos like AB, ABC")
    ...
    parser.add_argument("--message", default=f"auto deploy lora-v1 {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    args = parser.parse_args()

    # --- Step 1: Generate training data ---
    if not args.skip_distill:
        header(f"Distilling training data (tier={args.tier})")
        distill_args = ["--no-capture-output", "python", "-u", str(_HERE / "distill.py"), "--tier", args.tier]
        if args.max_c > 0:
            distill_args += ["--max-c", str(args.max_c)]
        conda_check(distill_args)
