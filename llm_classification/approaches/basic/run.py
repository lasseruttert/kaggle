#!/usr/bin/env python3
"""
Deploy basic approach: train → save → upload dataset → push kernel → poll status.
...
DATASET_DIR = Path(__file__).parent.parent.parent / "kaggle_dataset" / "bert-finetuned"
KERNEL_ID = "lasseruttert/bert-basic-inference"
...
def main() -> None:
    ...
    parser.add_argument("--message", default=f"auto deploy basic {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    if re.search(r"complete", status, re.IGNORECASE):
        ...
        print("  Check results at: https://www.kaggle.com/competitions/llm-classification-finetuning/submissions")
