#!/usr/bin/env python3
"""
Deploy LLM-CLS approach: train → save → upload dataset → push kernel → poll status.
...
DATASET_DIR = Path(__file__).parent.parent.parent / "kaggle_dataset" / "llm-cls-finetuned"
KERNEL_ID   = "lasseruttert/llm-cls-inference"
...
def main() -> None:
    ...
    parser.add_argument(
        "--message",
        default=f"auto deploy llm-cls {datetime.now().strftime('%Y-%m-%d %H:%M')}",
    )
