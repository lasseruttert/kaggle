"""
train.py — Full Fine-Tune Qwen2.5-1.5B 3-Class Classifier
Fine-tunes Qwen2.5-1.5B end-to-end (no LoRA) for sequence classification.
Smaller model enables full fine-tuning within the 16GB GPU budget.

Memory budget (16GB GPU):
  1.5B weights bfloat16      : ~3.0 GB
  AdamW optimizer states     : ~6.0 GB
  Activations (grad ckpt)    : ~0.5 GB
  Classification head + grads: ~0.1 GB
  Total                      : ~9.6 GB
...
LR              = 5e-5
WARMUP_RATIO    = 0.05
BATCH_SIZE      = 4       # 1.5B fits with headroom
...
def load_model(base_model_path: str):
    ...
    # Full fine-tune — no LoRA, all parameters get gradients
...
def main():
    ...
    for fold in range(FOLDS):
        ...
        tr_ds = LLMClsDataset(tr_df_aug, tokenizer)
        va_ds = LLMClsDataset(va_df, tokenizer)
        ...
        ckpt_path = os.path.join(CKPT_DIR, f"best_llm_cls_f{fold}.pt")
